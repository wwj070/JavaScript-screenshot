<template>
  <div class="content">
        ...
        <div style="background:green;" class="borderSet">
          <div style="background:blue;" class="borderSet">
            <div style="background:yellow;" class="borderSet">
              <div style="background:orange;" class="borderSet">33333333333333333333333333333333</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <h2 class="toCanvas" v-show="tocanvash2">
      <a href="javascript:void(0);" @click="tocanvas">转成canvas</a>
    </h2>
    <h2 v-show="topic">
      <a href="javascript:void(0);" @click="toPic">转成图片</a>
    </h2>
    <div v-show="parmas">
...
export default {
  ...
  methods: {
    ...
    save () {
      ...
      let w = (this.imgParmas.imgWidth
        ? this.imgParmas.imgWidth
        : this.canvasWidth
      ).toString() //图片宽度
      let h = (this.imgParmas.imgHeight
        ? this.imgParmas.imgHeight
        : this.canvasHeight
      ).toString() //图片高度
      let f = (this.imgParmas.filename
        ? this.imgParmas.filename
        : new Date().getTime()
      ).toString() //图片文件名
      // 调用Canvas2Image插件
      console.log(type)
